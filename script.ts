...
class Board {
    ...
    private setUpBoard(row: number, col: number) {
    ...
    private setUpBombs(row: number, col: number) {
    ...
    private getUniqueBombs(row: number, col: number) {
        ...
        const illegalPositions: Set<string> = new Set();
        ...
        for (let i = 0; i < this.rows; i++) {
            for (let j = 0; j < this.cols; j++) {
                ...
                if (currentCell.getType() == "default") {
                    ...
                    const numOfAdjacentBombs = this.checkAdjacentCellsForBombs(
                        i,
                        j
                    );
    ...
    private checkAdjacentCellsForBombs(row: number, col: number) {
        const directions = ["l", "r", "t", "b", "tr", "tl", "br", "bl"];
        let value = 0;
        directions.forEach((dir) => {
            ...
            const adjCell =
                ...
                this.boardArray[adjacentCellPosition[0]][
                    adjacentCellPosition[1]
                ];
    ...
    private getAdjacentCell(row: number, col: number, direction: string) {
        switch (direction) {
            case "l":
                if (col > 0) {
                    return [row, col - 1];
    ...
    private handleReveal(row: number, col: number) {
        ...
        if (currentCell.getValue() == 0) {
            ...
            const directions = ["l", "r", "t", "b", "tr", "tl", "br", "bl"];
            directions.forEach((dir) => {
                const adjacentCellPosition = this.getAdjacentCell(
                    row,
                    col,
                    dir
                );
